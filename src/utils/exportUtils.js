...
const ensureAutoTable = async () => {
  try {
    ...
    // Silent fail - will use fallback table generation
  }
};
...
const generatePdfReport = (
  ...
  // Title Section
  doc.setFontSize(24);
  doc.text("Climate Change Risk Assessment Report", pageWidth / 2, currentY, {
    align: "center",
  });
  currentY += 15;
  ...
  const summaryText =
    `This report presents a comprehensive Climate Change Risk Assessment for ${cityName}. ` +
  ...
  if (qualitativeScore !== null) {
    ...
    doc.setFontSize(11);
    doc.text(
      `Resilience Score: ${(qualitativeScore * 100).toFixed(1)}%`,
      horizontalMargin,
      currentY
    );
    currentY += 8;
    ...
    const qualitativeNote =
      "Note: The risk scores in this report have been adjusted based on the qualitative assessment results.";
    currentY = addWrappedText(
      qualitativeNote,
      horizontalMargin,
      currentY,
  ...
  const topRisks = [...ccraData]
    .sort((a, b) => {
      const scoreA = qualitativeScore
        ? calculateAdjustedRiskScore(a, qualitativeScore)
        : a["Risk Score"];
      const scoreB = qualitativeScore
        ? calculateAdjustedRiskScore(b, qualitativeScore)
        : b["Risk Score"];
  ...
  topRisks.forEach((risk, index) => {
    ...
    const riskScore = qualitativeScore
      ? calculateAdjustedRiskScore(risk, qualitativeScore)
      : risk["Risk Score"];
    ...
    currentY += 7;
    doc.text(
      `Climate Threat Score: ${risk["Climate Threat Score"].toFixed(2)}`,
      horizontalMargin + 10,
      currentY
    );
    currentY += 7;
    doc.text(
      `Exposure Score: ${risk["Exposure Score"].toFixed(2)}`,
      horizontalMargin + 10,
      currentY
    );
    currentY += 7;
    doc.text(
      `Sensitivity Score: ${risk["Sensitivity Score"].toFixed(2)}`,
      horizontalMargin + 10,
      currentY
    );
...
const renderDetailsGrid = (details, x, y, maxWidth, doc, defaultFont) => {
  ...
  details.slice(0, 4).forEach((detail, index) => {
    ...
    if (index % 2 === 0) {
      // Left column (index 0 or 2)
      currentX = leftColX;
      startY = isRow1 ? row1Y : row2Y;
    } else {
      // Right column (index 1 or 3)
      currentX = rightColX;
      startY = isRow1 ? row1Y : row2Y;
    }
...
const getReductionPotentialSafe = (action, t) => {
  ...
  if (
    ...
    typeof action.GHGReductionPotential === "object"
  ) {
    try {
...
export const exportToPDF = async (
  ...
  const addWrappedText = (
    ...
    return (
      y + lines.length * calculatedLineHeight + (fontStyle === "bold" ? 1 : 0)
    ); // Add small extra gap for bold
  };
  ...
  if (mitigationData && mitigationData.length > 0) {
    ...
    mitigationData.slice(0, 3).forEach((item, index) => {
      ...
      const sectorValue =
        joinToTitleCase(item.action.Sector || [], t) || t("na");
      const costValue = item.action.CostInvestmentNeeded
        ? t(item.action.CostInvestmentNeeded)
        : t("na");
      const timeValue = item.action.TimelineForImplementation
        ? t(getTimelineTranslationKey(item.action.TimelineForImplementation))
        : t("na");
  ...
  if (adaptationData && adaptationData.length > 0) {
    ...
    adaptationData.slice(0, 3).forEach((item, index) => {
      ...
      const potentialValue = item.action.AdaptationEffectiveness
        ? t(item.action.AdaptationEffectiveness)
        : t("na");
      const hazardValue =
        joinToTitleCase(item.action.Hazard || [], t) || t("na");
      const costValue = item.action.CostInvestmentNeeded
        ? t(item.action.CostInvestmentNeeded)
        : t("na");
      const timeValue = item.action.TimelineForImplementation
    ...
    const adaptationHeaders = [
      [t("pdf.priority"), t("pdf.actionName"), t("pdf.adaptationPotential")],
    ];
    const adaptationRows = adaptationData.map((item, index) => [
      index + 1,
      item.action.ActionName, // Assuming ActionName is language-agnostic or pre-translated
      item.action.AdaptationEffectiveness
        ? t(item.action.AdaptationEffectiveness)
        : t("na"),
    ]);
    ...
    if (doc.autoTable) {
      ...
      yPos =
        doc.autoTable.previous && doc.autoTable.previous.finalY
          ? doc.autoTable.previous.finalY + 10
          : yPos + adaptationRows.length * 15 + 20; // Fallback calculation
    } else {
      yPos = createSimpleTable(
        doc,
        adaptationHeaders,
  ...
  if (
    ...
    generatedPlans.forEach((planData, index) => {
      const planTitle = t("pdf.planTitle", {
        index: index + 1,
  ...
  for (let i = 1; i <= pageCount; i++) {
    doc.setPage(i);
    doc.setFontSize(9);
    doc.text(t("pdf.page", { i, pageCount }), pageWidth / 2, pageHeight - 10, {
      align: "center",
    });
  }
