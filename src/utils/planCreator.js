...
export const generateActionPlan = async ({ action, city }) => {
  try {
    ...
    console.log("Sending request to start plan creation:", {
      url: `${PLAN_CREATOR_URL}/plan-creator-legacy/start_plan_creation`,
      payload,
    });

    // Step 1: Start plan creation and get task ID
    const startResponse = await fetch(
      `${PLAN_CREATOR_URL}/plan-creator-legacy/start_plan_creation`,
      {
        method: "POST",
        headers: {
    ...
    while (status === "pending" || status === "running") {
      ...
      const statusResponse = await fetch(
        `${PLAN_CREATOR_URL}/plan-creator-legacy/check_progress/${task_id}`,
        {
          headers: {
            Accept: "application/json",
          },
    ...
    const planResponse = await fetch(
      `${PLAN_CREATOR_URL}/plan-creator-legacy/get_plan/${task_id}`,
      {
        headers: {
          Accept: "application/json",
        },
