...
const ClimateActions = ({
  ...
  const saveNewRanking = (type) => {
    const currentData = isAdaptation(type) ? adaptationData : mitigationData;

    const updatedRanking = currentData.map((action, index) => ({
      ...action,
    ...
    if (isAdaptation(type)) {
      setAdaptationData(updatedRanking);
    } else {
      setMitigationData(updatedRanking);
    }

    // Pass the updated data directly to the save function
    onSaveRankings(type, updatedRanking);
  };

  const setSelectedActionByIndex = (type) => (i) => {
    setSelectedAction(
      type === MITIGATION ? mitigationData[i] : adaptationData[i]
    );
  };

  const onSaveRankings = async (type, updatedData = null) => {
    setIsSaving(true);

    // Use the passed updatedData if available, otherwise use current state
    const dataToSave =
      updatedData || (isAdaptation(type) ? adaptationData : mitigationData);

    try {
      if (isAdaptation(type)) {
        await writeFile(selectedCity, dataToSave, ADAPTATION);
        if (!updatedData) {
          setAdaptationData(adaptationData);
        }
      } else {
        await writeFile(selectedCity, dataToSave, MITIGATION);
        if (!updatedData) {
              ...
              <div className="rounded-lg overflow-hidden">
                ...
                <ActionsTable
                  ...
                  onRowOrderChange={(updatedItems) => {
                    ...
                    if (isAdaptation(type)) {
                      setAdaptationData(newItems);
                    } else {
                      setMitigationData(newItems);
                    }
                  }}
                />
